...
for ($i=0;$i<$number_input;$i++)
{
	...
	{
		$file_id = $i / 6;
		$outputFile=$current_folder."/FmsQa_P$file_id".".C";
		$outputFilename="FmsQa_P$file_id";
		print $outputFile;
	...
	chomp($inputlist[$i]);
	my $fill = $inputlist[$i];
	#my $inputFilePrint = $folder."/condorjobResults/DiffJetNanoDstfms_0628fill/NanoDiffJetTree_fill$fill.root";
	my $inputFilePrint = $folder."/condorjobResults/NanoDiffJetTree_fill$fill.root";
	my $outputFilePrint = $folder."/root_result/FmsQaByFill/FmsQaFill$fill"."_0707a.root";

	print Output 'DiffJetAnaTreeQa("'."$inputFilePrint".'","'."$outputFilePrint".'","fms");'."\n";
