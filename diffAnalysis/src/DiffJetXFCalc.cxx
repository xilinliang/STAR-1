...
void DiffJetXFCalc(TString inFileName, TString outName, TString det)
{
	...
	if (nRpsTracks != 1) continue;
		...
		if (rpsTrack->GetThetaX() < -1.5 || rpsTrack->GetThetaX() > 5) continue;
	...
	for(Int_t j = 0; j <  jetEvent->GetNumberOfJets(); ++j)
	{
	    ...
	    if (skimEvent->GetSpinB() == 1) h2_U_xF_vs_phi->Fill(phi,xF);
	    if (skimEvent->GetSpinB() == -1) h2_D_xF_vs_phi->Fill(phi,xF);
